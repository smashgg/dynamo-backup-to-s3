...
DynamoBackup.prototype.uploadStream = function (tableName, stream, startTime, backupPath, callback) {
    var self = this;
    ...
    upload.send(function (err) {
        ...
        self.emit('end-backup', tableName, backupDuration, backupPath);
        return callback(err);
    });
};
...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
    ...
    if (self.writeToFile) {
        streamFn = 'write';
        stream = fs.createWriteStream(self.filePath + tableName + '.json');
    } else {
        streamFn = 'append';
        stream = new ReadableStream();
        self.uploadStream(tableName, stream, startTime, backupPath, callback);
    }
    ...
    self._copyTable(
        ...
        function (items) {
            items.forEach(function (item) {
                ...
                self.emit('item-processed', tableName, item);

                if (self.saveDataPipelineFormat) {
                    stream[streamFn](self._formatForDataPipeline(item), 'utf8');
                } else {
                    stream[streamFn](JSON.stringify(item), 'utf8');
                }
                stream[streamFn]('\n', 'utf8');
            });
        },
        function (err) {
            ...
            } else if (self.writeToFile) {
                stream.on('finish', function () {
                    var readStream = fs.createReadStream(self.filePath + tableName + '.json');
                    self.uploadStream(tableName, readStream, startTime, backupPath, callback);
                });
            }
